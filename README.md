...
The `services/submission_service.py` handles the submission of solutions by users. It now uses a persistent, MongoDB-based queue to make the submission process more robust.

#### Submission Flow with MongoDB Queue

1.  **Queueing:** When a user submits a solution, the submission is added to a `submissions_queue` collection in MongoDB.
2.  **Immediate Response:** The API immediately returns a `submission_id` to the user.
3.  **Background Worker:** A background worker continuously polls the `submissions_queue` for new submissions.
4.  **Grading:** When a new submission is picked up, it is graded by the `judge_service`.
5.  **GitHub Update:** After grading, the final results and submission files are created in the GitHub repository.
6.  **Cleanup:** The submission is removed from the `submissions_queue` in MongoDB. The `submissions` collection keeps a record of all submissions.

#### Submission ID Generation

To prevent a race condition where multiple submissions could get the same ID, the submission ID is now generated by MongoDB. When a new submission is made, it is inserted into the `submissions_queue` collection, and MongoDB automatically generates a unique `_id`. This `_id` is then used as the official `submission_id` for all subsequent processing and is returned to the user.

#### Important Note on Live Updates:

Due to the asynchronous nature of GitHub API writes and to prevent race conditions, live updates of test case results during grading are no longer pushed to GitHub. The `meta.json` file on GitHub will be updated with the final status and all test results once the grading process is complete.
...
Each problem can now have its own `validator.py` script located in the problem's directory (e.g., `data/contests/C1/problems/A/validator.py`). This script acts as a **custom judge** for the problem. The `judge_service` will execute this validator, passing the user's output and the test case input, to determine the verdict for each test case.
