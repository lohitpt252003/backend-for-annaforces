...
submissions_queue_collection = None

def init_app(app):
    global mongo_client, db, submissions_collection, submissions_queue_collection
    try:
        mongo_client = MongoClient(app.config["MONGO_URI"], serverSelectionTimeoutMS=5000)
        mongo_client.admin.command('ping')
        print("MongoDB connection successful in submission_service.")
        db = mongo_client["data"]
        submissions_collection = db["submissions"]
        submissions_queue_collection = db["submissions_queue"]
    except ConnectionFailure as e:
        print(f"Could not connect to MongoDB in submission_service: {e}")
        mongo_client = None

def _process_submission_from_queue():
    while True:
        if not mongo_client:
            print("No MongoDB connection in submission worker. Retrying in 10 seconds.")
            time.sleep(10)
            continue

        submission_doc = submissions_queue_collection.find_one_and_delete({})
        if submission_doc:
            new_submission_id = submission_doc["submission_id"]
            problem_id = submission_doc["problem_id"]
            username = submission_doc["username"]
            language = submission_doc["language"]
            code = submission_doc["code"]
            timestamp = submission_doc["timestamp"]
            ...
            # 3. Create submission files on GitHub
            ...
            # 4. Update the global submission count on GitHub
            global_submissions_meta_path = f"{GITHUB_SUBMISSIONS_BASE_PATH}/meta.json"
            global_submissions_meta_content, sha, global_submissions_meta_error = get_file(global_submissions_meta_path)

            if not global_submissions_meta_error:
                try:
                    ...
                    pass # Ignore if meta.json is invalid

            # 5. Insert final submission meta into submissions collection
            submissions_collection.insert_one(submission_meta_data)

            # 6. Update user problem status
            update_user_problem_status(username, problem_id, overall_status)

            print(f"[Submission Worker] Finished processing for submission {new_submission_id}")
        else:
            # No submissions in the queue, wait for a bit
            time.sleep(5)

def handle_new_submission(problem_id, username, language, code):
    if not mongo_client:
        return {"error": "Could not connect to the database to queue the submission."}
    ...
    submissions_queue_collection.insert_one(submission_doc)

    return {"message": "Submission received and is being processed.", "submission_id": new_submission_id}

# Start the background worker thread
submission_worker_thread = threading.Thread(target=_process_submission_from_queue)
submission_worker_thread.daemon = True
submission_worker_thread.start()